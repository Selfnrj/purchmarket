...
export default function AvtalSparade({ allAvtal }) {
  ...
  const tag = allAvtal?.edges?.filter(item => item.node.tags.edges[0]?.node.name === email).map((item) =>
    item.node.title
  );
  ...
  return (
    <div>
      {tag.length ? (
        allAvtal?.edges.filter(item => item.node.tags.edges[0]?.node.name === email).map((item) => (
          <AvtalCard 
            key={item.node.id}
            id={item.node.id}
